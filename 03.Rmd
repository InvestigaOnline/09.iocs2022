...
tab <- data %>%
    ...
    tab_cols(unvr(P1P2) %nest% unvr(R1C) %nest% unvr(R1D)) %>%
	tab_cells('|'=unvr(mrset_f(P3_))) %>%
	tab_stat_rpct() %>%
    tab_pivot(stat_position='outside_rows') %>%
    drop_rc() %>%
    tab_caption('Secciones en las que se fundamenta la satisfacción / recomendación')
