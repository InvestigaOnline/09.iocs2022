...
En la búsqueda de patrones, creemos oportuno probar la existencia de grupos 
diferenciados a partir de los indicadores básicos trabajados. Se trabaja 
únicamente con los indicadores comunes a todos los entrevistados sin diferenciación, 
P1, P2, P4, P5 y P17.
...
data01 <- subset(data, select = c(REGISTRO, KPIP1,KPIP2,KPIP4,KPIP5, KPIP17))
mh <- round(mahalanobis(data01[,-1], colMeans(data01[,-1]), cov(data01[,-1])),5)
pmh <- round(pchisq(mh, df=11, lower.tail=FALSE),5)
...
```{r}
data02 <- cbind(REGISTRO=data01[,1],clusterCut.03)
```

# Clúster K-medias

## Cálculo - Solución 3 grupos


Hacemos el análisis K-means para 3 grupos.

```{r}
grupos=3
#cluster kmeans
set.seed(311265)
kclusterCut.03 <- kmeans(data01[,-1], grupos, nstart = 10)
kclusterCut.03
```
...
colnames(data02) <- c('REGISTRO', 'grps')
datafinal <- merge(data, data02, by = "REGISTRO")
